...
#define REGS_HOLDING_ADDR_MAX 21 // This must be the amount of holding registers + 1, otherwise modbus support fails
...
#define MIN_SLAVE_VALUE 1
    ...
    typedef struct
    {
        ...
        uint16_t serial_number_in; // 40018 - Holding Register 18 - Use this register to write the serial number (first input the password)
        uint16_t sn_password;      // 40019 - Holding Register 19 - Writing the correct value into this register enables 1 serial number write for 15 seconds
        uint16_t sn_write_status;  // 40020 - Holding Register 20 - Status for the last serial number write attempt
